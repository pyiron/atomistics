...
class EnergyVolumeCurveCalculator(object):
    ...
    def analyse_structures(self, output_dict):
        ...
        if self.fit_type == "polynomial":
            self.fit_dict = self.fit_module.fit_polynomial(fit_order=self.fit_order)
        elif self.fit_type in [
            "birch",
            "birchmurnaghan",
            "murnaghan",
            "pouriertarantola",
            ...
            self.fit_dict = self.fit_module.fit_eos_general(fittype=self.fit_type)
        else:
            raise ValueError(
