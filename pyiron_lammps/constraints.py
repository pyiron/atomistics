        ...
        if c_dict['name'] == 'FixAtoms':
            ...
            if all(np.isin(c_dict['kwargs']['direction'], [0, 1])):
                if 'indices' in c_dict['kwargs'].keys():
                    fixed_atom_vector[c_dict['kwargs']['indices']] = np.array(c_dict['kwargs']['direction']).astype(
                        bool)
                elif 'a' in c_dict['kwargs'].keys():
                    fixed_atom_vector[c_dict['kwargs']['a']] = np.array(c_dict['kwargs']['direction']).astype(bool)
            else:
                raise ValueError(
