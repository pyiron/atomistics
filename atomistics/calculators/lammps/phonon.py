...
def generate_pylammps_trajectory(
    ...
    if temperature is not None:
        lmp.interactive_lib_command('fix int all nvt temp {0} {0} {1}'.format(temperature, thermostat_mass))
    ...
    if not np.array(lmp._interactive_library.gather_atoms("v", 1, 3)).any():
        ...
        lmp.interactive_lib_command('velocity all create {} 3627941 dist gaussian mom yes'.format(t))
...
def calc_molecular_dynamics_phonons_with_lammps(
    ...
    structure, positions, velocity, time, energy, simulation_cell, memmap = generate_pylammps_trajectory(
